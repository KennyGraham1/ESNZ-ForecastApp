...
const ThreeDVisualization = memo(function ThreeDVisualization({ earthquakes }: ThreeDVisualizationProps) {
    ...
    const chartOptions: Highcharts.Options = useMemo(() => {
        ...
        return {
            chart: {
                ...
                options3d: {
                    enabled: true,
                    alpha: 15,  // ✅ Increased for better initial viewing angle
                    beta: 30,
                    depth: 400,  // ✅ Increased from 250 to 400 for more pronounced Z-axis
                    viewDistance: 10,  // ✅ Increased from 5 to 10 for better perspective
                    fitToPlot: true,  // ✅ Changed to true to fit the 3D data properly
                    frame: {
                        visible: 'default',  // ✅ Make frame more visible
    ...
    useEffect(() => {
            ...
            const handleDragMove = (e: MouseEvent | TouchEvent) => {
                ...
                if (e instanceof TouchEvent && e.touches.length === 2) {
                    ...
                    if (initialPinchDistance > 0) {
                        ...
                        const newViewDistance = Math.max(2, Math.min(25, currentViewDistance / scale));
                ...
                // Update alpha (vertical rotation)
                const newAlpha = alpha + (moveCoords.y - posY) / sensitivity;
                if (chart.options.chart?.options3d) {
        ...
        const handleWheel = (e: WheelEvent) => {
            ...
            const newViewDistance = Math.max(2, Math.min(25, currentViewDistance + delta * zoomSensitivity));
